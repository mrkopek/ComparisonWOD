...
class ScanObjectNN(Dataset):
    ...
    def __getitem__(self, item):
        pointcloud = self.data[item][:self.num_points].astype("float32")
        label = self.label[item]
...
class WashingtonDataset(Dataset):
    def __init__(self, partition):
        self.data, self.label = load_rgbd_data(0,partition=="train")
...
def load_rgbd_data(trail,istrain):
    ...
    DATA_DIR=os.path.join(BASE_DIR,"./data/washington/wod_1024.h5")
    
    f=h5py.File(DATA_DIR,"r+")
    train_data = f["pointcloud"][:].astype("float32")
    instancelabel=f["instances"][:].astype("str")
    label=f["labels"][:].astype("str")
    testinstances=pd.read_csv(os.path.join(BASE_DIR,"./data/washington/testinstances2.csv"),delimiter=";",header=None).transpose()
